...
function QuestieNameplate:Initialize()
    if self.GlobalFrame == nil then
        self.GlobalFrame = CreateFrame("Frame")
        self.GlobalFrame:SetScript("OnUpdate", self.UpdateNameplate)
    end
end
...
function QuestieNameplate:NameplateCreated(token)
    ...
    if unitType == "Creature" then
        ...
        if icon then
            ...
            local f = self:getFrame(unitGUID)
...
function QuestieNameplate:UpdateNameplate(self)
    ...
    for guid, token in pairs(activeGUIDs) do
        ...
        if icon then
            local frame = self:getFrame(guid)
